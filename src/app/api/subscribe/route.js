...
export async function POST(req) {
  try {
    ...
    // 3) Notificación por correo (para ti)
    try {
      await resend.emails.send({
        from: process.env.ALERT_FROM || 'Casa Cora <onboarding@resend.dev>',
        to: process.env.ALERT_TO || 'agsolutions96@gmail.com',
      ...
      // No rompemos la suscripción si falló el email; solo registramos el error
    ...
    try {
      await resend.emails.send({
        from: process.env.ALERT_FROM || 'Casa Cora <onboarding@resend.dev>',
        to: normalizedEmail,
        subject: '¡Gracias por unirte a la Comunidad Casa Cora!',
        html: `
          <div style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif; line-height:1.6; color:#111">
            <p>Hola ${name || ''},</p>
            <p>Gracias por confiar en Casa Cora. Muy pronto recibirás invitaciones a rituales, círculos y sesiones abiertas.</p>
            <p>Con cariño,<br/>Fundación Casa Cora</p>
          </div>
        `,
      })
    } catch (e) {
